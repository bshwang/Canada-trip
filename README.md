...
2. `/itinerary` & `/itinerary/[day]` — 11일 일정 + 일자별 데이 노트(localStorage)
...
8. `/budget` — 일자별 지출 가계부 (CAD↔KRW 환율, 카테고리별 합계)
9. `/packing` — localStorage 체크박스
10. `/safety` — 곰·야생동물 안전 가이드 (Bear Spray 사용법, 비상연락)
11. `/info` — 세금/팁/비상/한식·한국 마트 + 환율 변환 위젯
12. `/more` — 나머지 메뉴 진입
